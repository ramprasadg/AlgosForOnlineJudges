...
public class PalindromeSubsequence {
    ...
    int find(String s, int start, int end) {
        ...
        largestPalindromeStartingAftertI[start] = Math.max(largestPalindromeStartingAftertI[start],
                maxAfterStart);
        return val;
    }
    ...
    int findMaxPairPalindrome(String s) {
        ...
        int max = 0;
        for (int i = 0; i < largestPalindromeEndingAtI.length; i++) {
            max = Math.max(largestPalindromeEndingAtI[i] * largestPalindromeStartingAftertI[i],
                    max);
        }
