...
public class SegmentTree {
    ...
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        /*
         * we will store left point as (position, houseIndex, height) right point as (position,
         * houseIndex, 0)
         */
    ...
    int updateTree(int index, int height, int node) {
        ...
        if (index % 2 == 0) {
            return tree[node] = Math.max(updateTree(index / 2, height, 2 * node + 1),
                    tree[2 * node + 2]);
        } else {
            return tree[node] = Math.max(updateTree(index / 2, height, 2 * node + 2),
                    tree[2 * node + 1]);
        }
    }
